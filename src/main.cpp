...
void Statistics(vector <Group> vgroups) {
    ...
    for (vector<Group>::iterator groupStatsIt = vgroups.begin(); groupStatsIt < vgroups.end(); ++groupStatsIt) {
        ...
        if (groupStatsIt->breederalive == 1)
            sumsqAlphaAge += groupStatsIt->vbreeder.alphaAge * groupStatsIt->vbreeder.alphaAge;

        if (groupStatsIt->breederalive == 1) sumAlphaAge2 += groupStatsIt->vbreeder.alphaAge2;
        if (groupStatsIt->breederalive == 1)
            sumsqAlphaAge2 += groupStatsIt->vbreeder.alphaAge2 * groupStatsIt->vbreeder.alphaAge2;
        ...
        if (groupStatsIt->breederalive == 1)
            sumsqBetaAge += groupStatsIt->vbreeder.betaAge * groupStatsIt->vbreeder.betaAge;
...
int main() {
    ...
    fout << "Generation" << "\t" << "Population" << "\t" << "Deaths" << "\t"
         ...
         << "\t"
         << "meanBeta" << "\t" << "meanBetaAge" << "\t" << "meanHelp" << "\t" << "meanDispersal" << "\t"
         << "Relatedness" << "\t"
         << "SD_GroupSize" << "\t" << "SD_Age" << "\t" << "SD_Alpha" << "\t" << "SD_AlphaAge" << "\t" << "SD_AlphaAge2"
         << "\t"
         << "SD_Beta" << "\t" << "SD_BetaAge" << "\t" << "SD_Help" << "\t" << "SD_Dispersal" << "\t" << "corr_Help_Disp"
         << endl;
    ...
    for (replica = 0; replica < MAX_NUM_REPLICATES; replica++) {
        ...
        for (gen = 1; gen <= NUM_GENERATIONS; gen++) {
            ...
            for (vector<Group>::iterator itAgeRelatedness = vgroups.begin();
                 itAgeRelatedness < vgroups.end(); ++itAgeRelatedness) {
